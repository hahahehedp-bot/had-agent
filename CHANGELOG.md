...
## [v13.2.5] - 2026-05-08
### Fixed
- **Flex-Fill Layout**: 채팅 모듈을 Flex 기반으로 재설계하여 입력창 바닥 고정 및 메시지 영역 독립 스크롤 구현.
- **Cache Annihilation**: 서비스 워커 및 엔트리 포인트 버전을 강제 업데이트하여 구버전 캐시 문제 해결.
- **Drawer Stability**: 서랍 내부 구조를 고정형으로 변경하여 불필요한 전체 스크롤 및 밀림 현상 방지.
