...
## [v13.2.6] - 2026-05-08
### Fixed
- **Space Maximization**: 비서창 헤더 상단 여백 축소 및 입력창 하단 밀착으로 대화 영역 공간 극대화.
- **Thread Optimization**: 메시지 리스트 하단 완충 지대(Spacer) 최소화로 스레드 가독성 향상.
- **UI Diet**: 전반적인 패딩 및 마진 축소로 모바일 화면 활용도 개선.
