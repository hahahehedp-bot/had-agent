...
## [v13.2.9] - 2026-05-08
### Fixed
- **Input Visibility Lock**: 특정 모바일 기기(Note 10 등)에서 입력창이 가려지는 문제를 해결하기 위해 유연한 높이 계산(`min-height: 0`) 적용.
- **Bottom Anchor Enhancement**: 하단 세이프 에어리어 여백을 상향하여 내비게이션 바와의 간섭 차단.
- **Flexbox Stability**: 서랍 및 모듈 레이아웃의 Flexbox 구조를 강화하여 화면 밖 탈출 현상 원천 봉쇄.
