...
## [v13.2.7] - 2026-05-08
### Fixed
- **Greeting Refinement**: 장황한 인사를 "리더님, 무엇을 도와드릴까요? 😊"로 대폭 축소하여 가독성 향상.
- **Layout Top-Start**: 메시지 리스트 상단 스페이서를 제거하여 첫 메시지부터 상단에 차곡차곡 배치.
- **Bottom Visibility Lock**: 입력창이 화면 밖으로 나가지 않도록 Flex-Fill 레이아웃 최종 안정화.
